...
let dir
before(async () => {
  dir = await mkdtemp(path.join(tmpdir(), 'decmpfs-copy-'))
})
after(async () => {
  if (dir) {
...
test('copyFile: FICLONE_FORCE clones on a cloning FS and round-trips', t => {
  const { src, compressed } = compressedSource('copyfile-force-src.node')
  const dest = path.join(dir, 'copyfile-force-dest.node')
  if (!compressed) {
    // A non-cloning/non-compressing FS legitimately cannot satisfy FORCE.
    assert.throws(
